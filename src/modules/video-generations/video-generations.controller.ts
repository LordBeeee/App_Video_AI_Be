...
export class VideoGenerationsController {
  ...
  async createVideo(
    ...
    if (!startImage && !startImageAssetId) {
      throw new BadRequestException('Start image là bắt buộc (file hoặc startImageAssetId)');
    }
    ...
    if (body.elementIds) {
      try {
        elementIds = JSON.parse(body.elementIds);
      } catch {
        throw new BadRequestException('elementIds không hợp lệ, phải là JSON array');
      }
      if (!Array.isArray(elementIds)) {
        throw new BadRequestException('elementIds phải là một array');
      }
